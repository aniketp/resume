...
\begin{itemize}[leftmargin=*, noitemsep]
  \item Created a parser for a log-structured distributed database to tokenize
      Office 365 backups as key-value byte streams.
  \item Reconstructed the SMTP Headers from the deserialized RocksDB dump to
      ZeroCopyBuffers using \textbf{C++ protobufs}.
  \item Developed a library to generate PST and EML format from parsed tokens
      using internal file-system interface over POSIX.
  \item Used MFCMAPI and libpst to create \textbf{BTree-on-Heap},
      \textbf{Heap-on-Nodes} and bitmap structures while generating PST.
  \item Feature integrated with Company's backup-indexing engine.
\end{itemize}

\vspace{5pt}
\runsubsection{New York Office, IIT Kanpur}
\descript{\hfill \textit{Feb'18 - Jun'18}}
